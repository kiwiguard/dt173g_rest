...
require '../classes/project.class.php';
...
// set new instance of Project class
$project = new Project();
...
switch($method) {
    ...
    case 'GET': // gets all stored data from database table
        if(isset($id)) {
            $result = $project->getOne($id); // if specific id is sent, get only that post from db
        } else {
            $result = $project->getAll(); // if no id is sent get all data from db table
            if (count($result) > 0) {
                http_response_code(200); //OK
            } else {
                http_response_code(400); //Not found
                $result = array('message' => 'No projects found'); // error message
            }
        }
        break;
    ...
    case 'POST' : // add post to db
        ...
        $project->pTitle = $d->pTitle;
        $project->pDescription = $d->pDescription;
        $project->pUrl = $d->pUrl;
        $project->pImg = $d->pImg;
